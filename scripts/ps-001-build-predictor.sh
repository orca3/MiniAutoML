...
if [ ! "$(docker ps -a | grep intent-classification-torch-predictor)" ]; then
  docker run --name intent-classification-torch-predictor --network orca3 --rm -d -p "${ICP_TORCH_PORT}":7070 -p "${ICP_TORCH_MGMT_PORT}":7071 \
    -v "${MODEL_CACHE_DIR}":/models \
    -v "$(pwd)/config/torch_server_config.properties":/home/model-server/config.properties \
    pytorch/torchserve:0.5.2-cpu torchserve --start --model-store /models
